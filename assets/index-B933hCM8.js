...
`,I=({cartItems:e=[]})=>(console.log(e),`
    ${e.length>0?`
        ...
        ${F()}
        `}
`);function L(e,t){return e.tagName===t?e:L(e.parentElement,t)}function R(e){return Number(e.replace(`원`,``).replace(/,/g,``))}const z=({message:e})=>`
<div id="info-toast" class="fixed bottom-10 left-1/2 transform -translate-x-1/2 z-50 opacity-0 transition-opacity duration-300">
...
`,B={products:[],filters:{},pagination:{},categoryLoading:!1,productLoading:!1,categories:{},productCountLimit:20,isEnd:!1},V=()=>{let{productCountLimit:e,pagination:t}=B,n=Number(new URL(window.location.href).searchParams.get(`limit`)||e),r=new URL(window.location.href).searchParams.get(`sort`)||`price_asc`,i=new URL(window.location.href).searchParams.get(`search`)||``,o=t.page,[s,c]=x.useState(`mounted`);return s===!1&&(B.productLoading=!0,B.categoryLoading=!0,Promise.all([w({limit:n,sort:r,page:o,search:i}).then(e=>{B.productLoading=!1,B.products=e.products,B.pagination=e.pagination,B.filters=e.filters,B.isEnd=e.pagination.total<=B.products.length}),E().then(e=>{B.categoryLoading=!1,B.categories=e})]).then(()=>{c(!0),b()})),`
    <div class="min-h-screen bg-gray-50">
      ...
      ${f({state:B})}
        ...
        <div class="mb-6">
          <div>
          <!-- 상품 개수 정보 -->
          ${B.productLoading?``:`<div class="mb-4 text-sm text-gray-600">
              총 <span class="font-medium text-gray-900">${B.pagination.total}개</span>의 상품
            </div>`}
            <!-- 상품 그리드 -->
            ${v({children:`
              ${(B.products??[]).length>0?`${B.products.map(e=>p(e)).join(``)}`:_.repeat(4)}
              `})}
...
${z({message:`장바구니에서 삭제되었습니다`})}
`};V.state=B,V.registerEvent=()=>{let e=document.getElementById(`limit-select`),t=document.getElementById(`sort-select`),n=document.getElementById(`search-input`),r=document.querySelectorAll(`.product-card`),i=document.querySelectorAll(`.add-to-cart-btn`),a=document.getElementById(`cart-icon-btn`),o=document.getElementById(`cart-modal-close-btn`),s=document.getElementById(`cart-modal-overlay`),c=document.querySelectorAll(`.quantity-decrease-btn`),l=document.querySelectorAll(`.quantity-increase-btn`),u=document.querySelectorAll(`.quantity-input`),d=document.getElementById(`cart-modal-total-price`),f=document.querySelectorAll(`.cart-item-price`),p=document.querySelectorAll(`.cart-item-lprice`),m=document.querySelectorAll(`.cart-item-remove-btn`),h=document.getElementById(`cart-modal-clear-cart-btn`);h&&h.addEventListener(`click`,()=>{x.setState(`cartItems`,[],{persist:!0}),b(),A({message:`장바구니가 비워졌습니다`,type:`info`})}),m.forEach(e=>{e.addEventListener(`click`,e=>{let t=L(e.target,`BUTTON`),n=t.dataset.productId;console.log(n);let r=x.getState(`cartItems`);r=r.filter(e=>e.productId!==n),x.setState(`cartItems`,r,{persist:!0}),b(),A({message:`장바구니에서 삭제되었습니다`,type:`info`})})}),c.forEach(e=>{e.addEventListener(`click`,e=>{let t=L(e.target,`BUTTON`),n=t.dataset.productId,r=Array.from(u).find(e=>e.dataset.productId===n);if(Number(r.value)>1){let e=Number(r.value)-1;r.value=e;let t=x.getState(`cartItems`);t=t.map(t=>t.productId===n?{...t,quantity:e}:t),x.setState(`cartItems`,t,{persist:!0});let i=Array.from(f).find(e=>e.dataset.productId===n),a=R(Array.from(p).find(e=>e.dataset.productId===n).textContent),o=a*e;i.innerHTML=o.toLocaleString(`ko-KR`)+`원`,d.textContent=(Number(R(d.textContent))-a).toLocaleString(`ko-KR`)+`원`}})}),l.forEach(e=>{e.addEventListener(`click`,e=>{let t=L(e.target,`BUTTON`),n=t.dataset.productId,r=Array.from(u).find(e=>e.dataset.productId===n);if(Number(r.value)){let e=Number(r.value)+1;r.value=e;let t=x.getState(`cartItems`);t=t.map(t=>t.productId===n?{...t,quantity:e}:t),x.setState(`cartItems`,t,{persist:!0});let i=Array.from(f).find(e=>e.dataset.productId===n),a=R(Array.from(p).find(e=>e.dataset.productId===n).textContent),o=a*e;i.innerHTML=o.toLocaleString(`ko-KR`)+`원`,d.textContent=(Number(R(d.textContent))+a).toLocaleString(`ko-KR`)+`원`}})}),a.addEventListener(`click`,()=>{x.setState(`isCartModalOpen`,!0),b()}),o&&o.addEventListener(`click`,()=>{x.setState(`isCartModalOpen`,!1),b()}),s&&s.addEventListener(`click`,()=>{x.setState(`isCartModalOpen`,!1),b()}),window.addEventListener(`keydown`,e=>{e.key===`Escape`&&(x.setState(`isCartModalOpen`,!1),b())}),r.forEach(e=>{e.addEventListener(`click`,()=>{C.push(`/product/${e.dataset.productId}`)})}),i.forEach(e=>{e.addEventListener(`click`,e=>{e.stopPropagation();let t=B.products.find(t=>t.productId===e.target.dataset.productId);k(t,1),A({message:`장바구니에 추가되었습니다`,type:`success`})})}),e.addEventListener(`change`,e=>{let t=new URL(window.location.href),n=D({...O(t.search),limit:e.target.value});C.push(`${t.pathname}?${n}`)}),t.addEventListener(`change`,e=>{let t=new URL(window.location.href),n=D({...O(t.search),sort:e.target.value});C.push(`${t.pathname}?${n}`)}),n.addEventListener(`keydown`,e=>{if(e.key!==`Enter`)return;let t=new URL(window.location.href),n=D({...O(t.search),search:e.target.value});C.push(`${t.pathname}?${n}`)});let g=document.getElementById(`product-list-end-indicator`),_=!1,v=new IntersectionObserver(e=>{e.forEach(e=>{if(e.isIntersecting&&!_){_=!0,B.pagination.page++,v.unobserve(g);let e=B.pagination.limit,t=B.filters.sort,n=B.pagination.page,r=B.filters.search;w({limit:e,sort:t,page:n,search:r}).then(e=>{B.productLoading=!1,B.products=[...B.products,...e.products],B.pagination=e.pagination,B.filters=e.filters,B.isEnd=e.pagination.total<=B.products.length}).then(()=>{_=!1,b()})}})},{threshold:.7});v.observe(g)};const H=`
...
`,q=({image:e,name:t,rating:n,reviewCount:r,price:i,stock:a,description:o})=>`
...
`,X={product:null,isDetailLoading:!1,isRelatedProductsLoading:!1,isCartAdded:!1,relatedProducts:[],cartItems:[]},Z=()=>{let[e,t]=x.useState(`mounted`),n=new URL(window.location.href).pathname.split(`/`),r=n[n.length-1];return e===!1&&(X.isDetailLoading=!0,T(r).then(async e=>{X.product=e,X.isDetailLoading=!1,t(!0),b()})),X.isRelatedProductsLoading=!0,X.product&&X.relatedProducts.length<=0&&new Promise(e=>setTimeout(e,2e3)).then(()=>{w({category1:X.product.category1,category2:X.product.category2}).then(e=>{X.relatedProducts=e.products.filter(e=>e.productId!==X.product.productId),X.isRelatedProductsLoading=!1,b()})}),`
    <div class="min-h-screen bg-gray-50">
      ${a({title:`상품 상세`,cartItemCount:x.getState(`cartItems`).length})}
    <main class="max-w-md mx-auto px-4 py-4">
      ${X.isDetailLoading?H:`
          <!-- 브레드크럼 -->
          ${J({category1:X.product.category1,category2:X.product.category2})}
            <!-- 상품 상세 정보 -->
            ${q({image:X.product.image,name:X.product.title,rating:X.product.rating,reviewCount:X.product.reviewCount,price:X.product.lprice,stock:X.product.stock,description:X.product.description})}`}
      <!-- 상품 목록으로 이동 -->
      <div class="mb-6">
      ...
      <!-- 관련 상품 -->
      ${X.isRelatedProductsLoading&&(X.relatedProducts??[]).length<=0?``:Y({products:X.relatedProducts})}
    </main>
    ${g}
  </div>
  ${j({message:`장바구니에 추가되었습니다`})}
  `};Z.registerEvent=()=>{let e=document.querySelector(`#quantity-decrease`),t=document.querySelector(`#quantity-increase`),n=document.querySelector(`#quantity-input`),r=document.querySelector(`#add-to-cart-btn`),i=document.querySelectorAll(`#related-product`);e&&e.addEventListener(`click`,()=>{let e=parseInt(n.value);e>1&&(n.value=e-1)}),t&&t.addEventListener(`click`,()=>{let e=parseInt(n.value);e<X.product.stock&&(n.value=e+1)}),r&&r.addEventListener(`click`,async()=>{let e=parseInt(n.value);k(X.product,e),A({message:`장바구니에 추가되었습니다`,type:`success`})}),i&&i.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.productId;C.push(`/product/${t}`),X.relatedProducts=[],x.setState(`mounted`,!1)})})};const Q=()=>r(async()=>{let{worker:e}=await import(`./browser-CcyfQrG1.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`,serviceWorker:{url:`${i}/mockServiceWorker.js`}}));function $(){x.subscribe({update:()=>{b({state:x.state})}}),C.addRoute(`/`,{regex:RegExp(`^${i}/?$`),page:V}),C.addRoute(`/product/:id`,{regex:RegExp(`^${i}/product/\\d+$`),page:Z}),x.notify()}Q().then($);